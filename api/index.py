...
# In-memory fallback stores for the persistent Patient Database, used when
# Supabase credentials are not configured (or the "patients"/"patient_visits"
# tables have not been created yet). Keyed by the durable patient_code
# (e.g. "MRN-3F9A21B0"), which is distinct from the ephemeral "PT-xxxx" IDs
# regenerated by every triage simulation batch.
PATIENTS_STORE = {}
VISITS_STORE = {}


def generate_patient_code():
    """Generates a durable, human-scannable Medical Record Number style identifier."""
...
PATIENT_FIELDS = [
    "full_name", "age", "gender", "date_of_birth", "phone", "email", "address",
...
def list_patients():
    """
    Lists patients in the persistent Patient Database, optionally filtered by a
    free-text search term matched against full_name or patient_code.
    """
    try:
        ...
        if supabase_client:
            try:
                q = supabase_client.table("patients").select("*")
                if query:
                    q = q.or_(f"full_name.ilike.%{query}%,patient_code.ilike.%{query}%")
                res = q.order("updated_at", desc=True).execute()
                results = res.data or []
            except Exception as e:
                print(f"Supabase patients list note: {e}")

        if results is None:
            lowered = query.lower()
            results = [
                p for p in PATIENTS_STORE.values()
                if not lowered or lowered in (p.get("full_name") or "").lower()
                or lowered in (p.get("patient_code") or "").lower()
            ]
        ...
        return jsonify({"error": str(e), "status": "patients_list_failed"}), 500


@app.route('/api/patients', methods=['POST'])
def create_patient():
    """Creates a new persistent patient record in the Patient Database."""
    try:
        data = request.json or {}
        ...
        timestamp = now_iso()
        record = {"patient_code": generate_patient_code(), "created_at": timestamp, "updated_at": timestamp}
        for field in PATIENT_FIELDS:
            record[field] = data.get(field) if field != "full_name" else full_name
        if not isinstance(record.get("flags"), list):
            record["flags"] = []

        if supabase_client:
            try:
                supabase_client.table("patients").insert(record).execute()
            except Exception as e:
                print(f"Supabase patient create note: {e}")
                PATIENTS_STORE[record["patient_code"]] = record
        else:
            PATIENTS_STORE[record["patient_code"]] = record
        ...
        return jsonify({"error": str(e), "status": "patient_create_failed"}), 500


@app.route('/api/patients/<patient_code>', methods=['GET'])
def get_patient(patient_code):
    """Returns a patient's full profile plus their visit/triage history."""
    try:
        patient = None
        visits = []
        ...
        if supabase_client:
            try:
                res = supabase_client.table("patients").select("*").eq("patient_code", patient_code).limit(1).execute()
                if res.data:
                    patient = res.data[0]
                visits_res = supabase_client.table("patient_visits").select("*").eq("patient_code", patient_code).order("created_at", desc=True).execute()
                visits = visits_res.data or []
            except Exception as e:
                print(f"Supabase patient fetch note: {e}")

        if patient is None:
            patient = PATIENTS_STORE.get(patient_code)
            visits = VISITS_STORE.get(patient_code, [])
        ...
        return jsonify({"error": str(e), "status": "patient_fetch_failed"}), 500


@app.route('/api/patients/<patient_code>', methods=['PATCH'])
def update_patient(patient_code):
    """
    Applies an in-place edit to a patient's profile. The client is responsible
    for confirming with the user before calling this when 2+ fields changed at
    once; this endpoint simply persists whatever fields are included.
    """
    try:
        data = request.json or {}
        ...
        if supabase_client:
            try:
                res = supabase_client.table("patients").update(updates).eq("patient_code", patient_code).execute()
                if res.data:
                    updated = res.data[0]
            except Exception as e:
                print(f"Supabase patient update note: {e}")

        if updated is None:
            existing = PATIENTS_STORE.get(patient_code)
            if existing is None:
                return jsonify({"error": "Patient not found"}), 404
            existing.update(updates)
            updated = existing

        return jsonify({"patient": updated})

    except Exception as e:
        return jsonify({"error": str(e), "status": "patient_update_failed"}), 500


@app.route('/api/patients/<patient_code>/visits', methods=['POST'])
def add_patient_visit(patient_code):
    """
    Attaches a visit record (a triage-simulator summary or a diagnosis/care
    plan snapshot) to a patient's history so ER staff can review prior visits.
    """
    try:
        data = request.json or {}
        record = {
            ...
            "visit_type": data.get("visit_type", "manual"),
        ...
        if supabase_client:
            try:
                supabase_client.table("patient_visits").insert(record).execute()
            except Exception as e:
                print(f"Supabase visit create note: {e}")
                VISITS_STORE.setdefault(patient_code, []).insert(0, record)
        else:
            VISITS_STORE.setdefault(patient_code, []).insert(0, record)

        return jsonify({"visit": record})

    except Exception as e:
        return jsonify({"error": str(e), "status": "visit_create_failed"}), 500
